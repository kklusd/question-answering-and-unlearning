...
class LoadSQuADQuestionAnsweringDataset(LoadSingleSentenceClassificationDataset):
    ...
    def __init__(self, doc_stride=64,
        ...
        self.forget_ids = {}
        if forget_ids_path is not None:
            with open(forget_ids_path, "r") as f:
                self.forget_ids = json.load(f)
    ...
    def generate_batch(self, data_batch):
        ...
        for item in data_batch:
            ...
            if self.unlearning:
                qa_id = item[7]
                if qa_id in self.forget_ids:
                    batch_forget_labels.append(1)
                else:
                    batch_forget_labels.append(0)
