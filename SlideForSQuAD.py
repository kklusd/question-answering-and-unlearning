...
class ModelConfig:
    def __init__(self):
        ...
        self.unlearn_model_path = os.path.join(self.model_save_dir, 'slide-badk_model.pt')
        ...
        logger_init(log_file_name='label_slide', log_level=logging.DEBUG,
    ...
    def train(config):
        ...
        for epoch in range(config.epochs):
            ...
            for idx, (batch_input, batch_seg, batch_label, _, _, _, _, batch_forget_labels) in enumerate(train_iter):
                ...
                slide = np.random.uniform(-1, 1, b)
                anwser_len = end_positions - start_positions
                slide = np.rint(slide * anwser_len.numpy())
                slide = torch.from_numpy(slide) * batch_forget_labels
                start_positions = start_positions + slide
                end_positions = end_positions + slide
                batch_input = batch_input.to(config.device)  # [src_len, batch_size]
                batch_seg = batch_seg.to(config.device)
                start_positions = start_positions.long().to(config.device)
                end_positions = end_positions.long().to(config.device)
    ...
    if only_for_eval:
        ...
        train_iter, test_iter, val_iter, forget_iter = \
            data_loader.load_train_val_test_data(train_file_path=config.train_file_path,
                                                 test_file_path=config.test_file_path,
                                                 forget_file_path=config.forget_file_path,
                                                 val_file_patt=config.val_file_path,
                                                 unlearn=True,
                                                 only_test=False)
...
def evaluate(data_iter, model, device, PAD_IDX, inference=False):
    ...
    with torch.no_grad():
        ...
        for batch_input, batch_seg, batch_label, batch_qid, _, batch_feature_id, _, _ in tqdm(data_iter):
...
if __name__ == '__main__':
    ...
    main(config=model_config, only_for_eval=True)
